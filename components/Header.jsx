...
const Header = () => {
  ...
  return (
    <header className="backdrop-blur-lg shadow-lg sticky top-0 z-99 flex flex-row w-screen">
        {/* logo */}
      <div className="flex w-3/4 md:w-1/2 lg:w-1/4  bg-black/80 justify-center items-center">
          <Link href="/">
          <Image src={"/images/logo-ksgh.webp"} alt='Kingh Sukh Guest House' width="186" height="115" /></Link>
      </div>
    
      <div className="bg-white/80 flex w-1/4 md:w-1/2 lg:w-3/4">
        ...
        <nav className="hidden lg:flex items-center space-x-6 text-lg">
          {navItems.map((item) =><Link key={item.href} href={item.href} className="text-gray-700 hover:text-[#D4AF37]">{item.name}</Link>) }
        </nav>
        {/* cta button */}
